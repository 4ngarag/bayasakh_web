    ...
    <div class="py-12">
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
            ...
            <div class="flex justify-ends m-2 p-2">
                <a href="{{ route('admin.products.index') }}" class="px-4 py-2 bg-indigo-500 hover:bg-indigo-700 rounded-lg text-white">Буцах</a>
            </div>
            ...
            <form method="POST" action="{{ route('admin.products.store') }}" enctype="multipart/form-data">
                ...
                <div class="flex flex-wrap -mx-3 mb-6">
                    ...
                    <div class="w-full md:w-1/2 px-3">
                        <label for="brandsub" class="block text-sm font-medium text-gray-700"> Брэнд </label>
                        <div class="mt-1">
                            <select id="brandsub_id" name="brandsub_id" class="form-multiselect block w-full mt-1">
                                @foreach ($brandsubs as $brandsub)
                                    <option value="{{ $brandsub->id }}">{{ $brandsub->name }}</option>
                                @endforeach
                            </select>
                        </div>
                    </div>
                ...
                <div class="flex flex-wrap -mx-3 mb-6">
                    ...
                    <div class="w-full md:w-1/2 px-3">
                        ...
                        @error('start_year')
                            <div class="text-sm text-red-400">{{ $message }}</div>
                        @enderror
                        <label for="logo" class="block text-sm font-medium text-gray-700">Лого</label>
                        <div class="mt-1">
                        ...
                        <div class="mt-1">
                            <input type="file" id="slider_image" name="slider_image" class="block w-full transition duration-150 ease-in-out appearance-none bg-white border border-gray-400 rounded-md py-2 px-3 text-base leading-normal transition duration-150 ease-in-out sm:text-sm sm:leading-5 @error('name') border-red-400 @enderror" />
                        </div>
                        @error('slider_image')
                            <div class="text-sm text-red-400">{{ $message }}</div>
                        @enderror
