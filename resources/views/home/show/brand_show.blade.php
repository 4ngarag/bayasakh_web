...
<section class="relative py-24 py-16">
    <div class="container relative disable-select">
        ...
        <!-- <div class="grid lg:grid-cols-6 md:grid-cols-3 grid-cols-2 mt-8 gap-[4px]">
            <div class="group relative overflow-hidden hover:shadow-lg hover:dark:shadow-gray-800 rounded-md duration-500 p-6 text-center">
                <img src="{{ Storage::url($brand->logo) }}" class="rounded-full shadow-md dark:shadow-gray-800 h-20 w-20 block mx-auto mb-2" alt="">

                <a href="/brands/{{ $brand->id }}" class="font-semibold hover:text-indigo-600 text-lg">Бүгд</a>
            </div>
            @php $brandsubs = App\Models\BrandSub::select('id','name','logo')->where('brand_id',$brand->id)->get() @endphp
            @foreach($brandsubs as $brandsub)
            <div class="group relative overflow-hidden hover:shadow-lg hover:dark:shadow-gray-800 rounded-md duration-500 p-6 text-center">
                <img src="{{ Storage::url($brandsub->logo) }}" class="rounded-full shadow-md dark:shadow-gray-800 h-20 w-20 block mx-auto mb-2" alt="">

                <a href="/brands/{{ $brand->id }}/{{ $brandsub->id }}" class="font-semibold hover:text-indigo-600 text-lg">{{ $brandsub->name }}</a>
            </div>
            @endforeach
        </div> -->
    ...
    <div class="container relative md:mt-24 mt-16">
        ...
        @php $products = App\Models\Product::inRandomOrder()->select('id','name','logo','brandsub_id')->where('brand_id',$brand->id)->get() @endphp
