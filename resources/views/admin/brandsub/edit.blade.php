<x-admin-layout>
    ...
    <div class="py-12">
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
            ...
            <form method="POST" action="{{ route('admin.brandsub.update', $brandsub->id) }}" enctype="multipart/form-data">
                @csrf
                @method('PUT')
                ...
                <div class="flex flex-wrap -mx-3 mb-6">
                    <div class="w-full md:w-1/2 px-3 mb-6 md:mb-0">
                        ...
                        <div class="mt-1">
                            <input type="text" id="name" name="name" value="{{ $brandsub->name }}" class="block w-full transition duration-150 ease-in-out appearance-none bg-white border border-gray-400 rounded-md py-2 px-3 text-base leading-normal transition duration-150 ease-in-out sm:text-sm sm:leading-5" />
                        </div>
                    </div>
                    <div class="w-full md:w-1/2 px-3">
                        <label for="companies" class="block text-sm font-medium text-gray-700">Brand</label>
                        <div class="mt-1">
                            <select id="brand_id" name="brand_id" class="form-multiselect block w-full mt-1">
                                <option value="{{ $brandsub->brand->id }}">{{ $brandsub->brand->name }}</option>
                                @foreach ($brands as $brand)
                                    <option @if($brandsub->brand_id == $brand->id) hidden @endif value="{{ $brand->id }}">{{ $brand->name }}</option>
                                @endforeach
                            </select>
                        </div>
                    </div>
                ...
                <div class="flex flex-wrap -mx-3 mb-6">
                    <div class="w-full md:w-1/2 px-3 mb-6 md:mb-0">
                        ...
                        <div class="mt-1">
                            <textarea id="texteditor" name="intro_en" rows="3" class="shadow-sm focus:ring-indigo-500 appearance-none bg-white border border-gray-400 rounded-md py-2 px-3 text-base leading-normal transition duration-150 ease-in-out focus:border-indigo-500 block w-full sm:text-sm border-gray-300 rounded-md">{{ $brandsub->intro_en }}</textarea>
                        </div>
                    </div>
                    <div class="w-full md:w-1/2 px-3">
                        <label for="intro_mn" class="block text-sm font-medium text-gray-700">Intro (MN)</label>
                        <div class="mt-1">
                            <textarea id="texteditor" name="intro_mn" rows="3" class="shadow-sm focus:ring-indigo-500 appearance-none bg-white border border-gray-400 rounded-md py-2 px-3 text-base leading-normal transition duration-150 ease-in-out focus:border-indigo-500 block w-full sm:text-sm border-gray-300 rounded-md">{{ $brandsub->intro_mn }}</textarea>
                        </div>
                    </div>
                </div>
                ...
                <div class="flex flex-wrap -mx-3 mb-6">
                    <div class="w-full md:w-1/2 px-3 mb-6 md:mb-0">
                        ...
                        <div class="mt-1">
                            <textarea id="texteditor" name="intro_ru" rows="3" class="shadow-sm focus:ring-indigo-500 appearance-none bg-white border border-gray-400 rounded-md py-2 px-3 text-base leading-normal transition duration-150 ease-in-out focus:border-indigo-500 block w-full sm:text-sm border-gray-300 rounded-md">{{ $brandsub->intro_ru }}</textarea>
                        </div>
                    </div>
                    <div class="w-full md:w-1/2 px-3">
                        <label for="start_year" class="block text-sm font-medium text-gray-700">Брэнд эсэх</label>
                        <div class="mt-1">
                            <select id="not_brandsub" name="not_brandsub" class="form-multiselect block w-full mt-1">
                                @if($brandsub->not_brand == 0)
                                <option value="0" selected>Тийм</option>
                                <option value="1">Үгүй</option>
                                @elseif($brandsub->not_brand == 1)
                                <option value="1" selected>Үгүй</option>
                        ...
                        <div>
                            <img class="w-32 h-32" src="{{ Storage::url($brandsub->logo) }}">
                        </div>
                        <div class="mt-1">
                        ...
                        <div>
                            <img class="w-32 h-32" src="{{ Storage::url($brandsub->slider_image) }}">
                        </div>
                        <div class="mt-1">
